(function($){
	...
	$(document).ready(function() {
		...
		$(window).resize(function() {
			if (moduleHero.length > 0) {
				if (moduleHero.hasClass('js-fullheight')) {
					moduleHero.height($(window).height() * 0.40);
				} else {
					moduleHero.height($(window).height() * 0.4);
				}
			}
		}).resize();
		...
		$('a[href="#bio"]').on('click', function(e) {
			console.log(e.target);
			var el = document.getElementById('bio');
			var elOffset = el.offsetTop;
			$('html, body').animate({ scrollTop: elOffset }, 'slow');
			return false;
		});

		/* ---------------------------------------------- /*
		 * Scroll Music sections
		/* ---------------------------------------------- */
		...
		$('a[href="#gigs-module"]').on('click', function(e) {
			console.log(e.target);
			var el = document.getElementById('gigs-module');
			var elOffset = el.offsetTop;
			$('html, body').animate({ scrollTop: elOffset }, 'slow');
			return false;
		});
