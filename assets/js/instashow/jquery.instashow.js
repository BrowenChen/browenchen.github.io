...
"use strict";
var $ = require('./jquery');
var Analytics = function (enable, version) {
    var self = this;
    self.enable = !!enable;
    self.version = version;
    self.initialize();
};
$.extend(Analytics, { URI: 'https://a.elfsight.com/collect' });
Analytics.prototype = function () {
};
$.extend(Analytics.prototype, {
    ...
    send: function (name, extra) {
        var self = this;
        if (!self.enable || !name || $.type(name) !== 'string') {
            return;
        }
        if (extra) {
        ...
        var $iframe = $('<iframe></iframe>', {
                src: Analytics.URI + '?' + $.param(params),
                css: {
                    position: 'absolute',
                    overflow: 'hidden',
                    left: '-9999px'
        ...
        $iframe.appendTo(document.body).on('load', function () {
            $iframe.remove();
        });
    }
});
...
"use strict";
var $ = require('./jquery'), u = require('./u'), defaults = require('./defaults'), Instapi = require('./instapi'), Gallery = require('./gallery'), Popup = require('./popup'), views = require('./views'), Lang = require('./lang'), Analytics = require('./analytics');
...
$.extend(Core, { VERSION: '1.5.0 Nuwa' });
Core.prototype = function () {
};
$.extend(Core.prototype, {
    initialize: function () {
        var self = this;
        self.analytics = new Analytics(self.options.analytics, Core.VERSION);
        self.analytics.send('init', self.id);
    ...
    showError: function (message) {
        var self = this;
        self.analytics.send('error', message);
        var $media = $(views.error({ message: message }));
        if (self.gallery) {
            self.gallery.puzzle();
            $media.appendTo(self.gallery.$root);
        } else {
            $media.insertBefore(self.$element);
        }
    }
});
module.exports = Core;
},{"./analytics":1,"./defaults":5,"./gallery":7,"./instapi":9,"./jquery":21,"./lang":22,"./popup":25,"./u":28,"./views":29}],5:[function(require,module,exports){
"use strict";
module.exports = {
    clientId: null,
    accessToken: null,
...
},{"./api":2,"./core":4,"./defaults":5,"./instashow":20,"./jquery":21}],7:[function(require,module,exports){
"use strict";
...
$.extend(Gallery.prototype, {
    ...
    watchScroll: function () {
        ...
        self.$root.on('wheel', function (e) {
            ...
            if (self.isFreeMode()) {
                ...
                scrolling = true;
                if (dir === 1 && self.hasNextView()) {
                    targetViewId = self.activeViewId + 1;
                } else if (dir === -1 && self.hasPreviousView()) {
                    targetViewId = self.activeViewId - 1;
                } else {
                    scrolling = false;
                    return;
                ...
                progress = self.getViewStartProgress(self.$viewsList.eq(targetViewId));
                self.translate(progress, true).done(function () {
                    scrolling = false;
                });
                self.setProgress(progress);
    ...
    watch: function () {
        ...
        self.$root.on('initialized.instaShow', function () {
            ...
            if (self.hasNextView() && self.$viewsList.length - 1 === id) {
                self.addView();
            }
            if (self.options.arrowsControl) {
                self.$arrowNext.toggleClass('instashow-gallery-control-arrow-disabled', !self.hasNextView());
                self.$arrowPrevious.toggleClass('instashow-gallery-control-arrow-disabled', !self.hasPreviousView());
            }
...
},{"./auto-rotator":3,"./grid":8,"./jquery":21,"./loader":23,"./move-control":24,"./scrollbar":26,"./translations":27,"./u":28,"./views":29}],8:[function(require,module,exports){
"use strict";
var $ = require('./jquery');
...
$.extend(Grid.prototype, {
    ...
    calculate: function () {
        ...
        if (self.autoHeight) {
            self.cellSize = Math.ceil((self.viewWidth - self.gutter) / self.columns);
            self.height = self.viewHeight = self.cellSize * self.rows + self.gutter;
            self.viewWidth = self.cellSize * self.columns + self.gutter;
        } else {
            if (self.viewRatio > 1) {
                self.cellSize = Math.floor((self.viewHeight - self.gutter) / self.rows);
            } else {
                self.cellSize = Math.floor((self.viewWidth - self.gutter) / self.columns);
            }
            self.viewWidth = Math.floor(self.cellSize * self.columns + self.gutter);
            self.viewHeight = Math.floor(self.cellSize * self.rows + self.gutter);
        }
...
$.extend(Instapi, {
    ...
    parseAnchors: function (str) {
        return str.replace(/(@|#)([^\s#@]+)/g, function (str, type, name) {
            var uri = '';
            switch (type) {
            case '@':
            ...
            return '<a href="' + uri + '" target="_blank">' + str + '</a>';
        });
    }
});
...
},{"./instapi/cache-provider":10,"./instapi/client":11,"./instapi/complex-media-fetcher":12,"./instapi/specific-media-fetcher":16,"./instapi/tag-media-fetcher":17,"./instapi/user-media-fetcher":18,"./jquery":21}],10:[function(require,module,exports){
"use strict";
var $ = require('../jquery');
...
},{"../jquery":21,"../u":28}],12:[function(require,module,exports){
"use strict";
var $ = require('../jquery');
...
module.exports = function (gallery) {
    ...
    var move = function (e) {
        ...
        var hasNext = gallery.hasView(self.activeViewId + 1);
        var hasPrevious = gallery.hasView(self.activeViewId - 1);
    ...
    return {
        watch: function () {
            ...
            if (gallery.options.scrollControl) {
...
$.extend(Popup.prototype, {
    ...
    createMedia: function (media) {
        ...
        $media.find('.instashow-popup-media-picture img').one('load', function () {
    ...
    hasNextMedia: function () {
        var self = this;
        return self.hasMedia(self.currentMedia + 1) || self.core.gallery.hasNextView();
    },
    hasPreviousMedia: function () {
        var self = this;
        return self.hasMedia(self.currentMedia - 1);
    },
    moveToNextMedia: function () {
        ...
        } else if (self.core.gallery.hasNextView()) {
    ...
    moveToPreviousMedia: function () {
        var self = this;
        return self.moveToMedia(self.currentMedia - 1);
    },
...
},{"./instapi":9,"./instapi/media":14,"./jquery":21,"./u":28,"./views":29}],26:[function(require,module,exports){
"use strict";
...
$.extend(Scrollbar.prototype, {
    ...
    fit: function () {
        ...
        var sliderSize = size / self.gallery.$viewsList.length;
...
views['gallery']['arrows'] = Handlebars.template({
    ...
    'main': function (depth0, helpers, partials, data) {
        return '<div class="instashow-gallery-control-arrow instashow-gallery-control-arrow-previous"></div><div class="instashow-gallery-control-arrow instashow-gallery-control-arrow-next"></div>';
    },
    'useData': true
});
...
views['popup']['media'] = Handlebars.template({
    ...
    '29': function (depth0, helpers, partials, data) {
        ...
        return '<div class="instashow-popup-media-info-comments-item"> <a href="https://instagram.com/' + alias2(alias1((stack1 = depth0 != null ? depth0.from : depth0) != null ? stack1.username : stack1, depth0)) + '" target="blank">' + alias2(alias1((stack1 = depth0 != null ? depth0.from : depth0) != null ? stack1.username : stack1, depth0)) + '</a> ' + ((stack1 = (helper = (helper = helpers.text || (depth0 != null ? depth0.text : depth0)) != null ? helper : helpers.helperMissing, typeof helper === 'function' ? helper.call(depth0, {
            'name': 'text',
            'hash': {},
            'data': data
